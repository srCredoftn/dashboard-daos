...
// Helper: attempt to obtain a persisted notification repository (mongo or in-memory)
async function getNotificationRepo() {
  try {
    const mod = await import("../repositories/mongoNotificationRepository");
    const RepoClass = (mod as any).MongoNotificationRepository;
    if (RepoClass) return new RepoClass();
  } catch (e) {
    // ignore
  }
  try {
    const mod = await import("../repositories/memoryNotificationRepository");
    const RepoClass = (mod as any).MemoryNotificationRepository;
    if (RepoClass) return new RepoClass();
  } catch (e) {
    // ignore
  }
...
class InMemoryNotificationService {
  ...
  private async persistNotification(item: ServerNotification) {
    try {
      ...
      logger.info("Notification persisted", "NOTIF", { type: item.type });
    } catch (e) {
      logger.warn(
        "Failed to persist notification (continuing in-memory)",
        "NOTIF",
        {
  ...
  private async mirrorEmail(item: ServerNotification) {
    // Skip mirroring for internal/system-only notifications
    if (item?.data && (item as any).data?.skipEmailMirror) return;

    const subject = item.title || "Notification";

    // Body: only the message, no duplicated title, no "Détails" section
    ...
    try {
      if (item.recipients === "all") {
        ...
        logger.info("Email mirror (diffusion) envoyé avec succès", "MAIL", {
          type: item.type,
        });
        return;
      }
      ...
      if (emails.length === 0) {
        logger.info("Email mirror: aucun destinataire trouvé (skip)", "MAIL", {
          type: item.type,
        });
        return;
      }
      ...
      logger.info("Email mirror envoyé avec succès", "MAIL", {
        type: item.type,
      });
    } catch (e) {
      const err: any = e;
      ...
      logger.error("Email mirroring failed after retries", "MAIL", {
        message: String((e as Error)?.message),
        code,
        type: item.type,
      });
  ...
  markRead(userId: string, notifId: string) {
    ...
    (async () => {
      try {
        ...
        logger.warn("Failed to persist markRead", "NOTIF", {
          message: String((e as Error)?.message),
        });
      }
    })();
  ...
  markAllRead(userId: string) {
    ...
    (async () => {
      try {
        ...
        logger.warn("Failed to persist markAllRead", "NOTIF", {
          message: String((e as Error)?.message),
        });
      }
    })();
  ...
  clearAll() {
    ...
    (async () => {
      try {
        ...
        logger.warn("Failed to persist clearAll", "NOTIF", {
          message: String((e as Error)?.message),
        });
      }
    })();
