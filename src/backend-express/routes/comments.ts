...
router.get("/dao/:daoId", authenticate, async (req, res) => {
  try {
    ...
    logger.error("Error getting DAO comments", "COMMENTS");
...
router.get("/dao/:daoId/task/:taskId", authenticate, async (req, res) => {
  try {
    ...
    logger.error("Error getting task comments", "COMMENTS");
...
router.post("/", authenticate, async (req, res) => {
  try {
    ...
    // Broadcast notification to all users + MailBus
    try {
    ...
    logger.error("Error adding comment", "COMMENTS");
...
router.put("/:id", authenticate, async (req, res) => {
  try {
    ...
    logger.error("Error updating comment", "COMMENTS");
...
router.delete("/:id", authenticate, async (req, res) => {
  try {
    ...
    logger.error("Error deleting comment", "COMMENTS");
    if ((error as Error).message.toLowerCase().includes("unauthorized")) {
      return res.status(403).json({ error: (error as Error).message });
    }
...
router.get("/recent", authenticate, async (req, res) => {
  try {
    ...
    logger.error("Error getting recent comments", "COMMENTS");
