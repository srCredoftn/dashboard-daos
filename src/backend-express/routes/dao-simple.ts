...
router.get("/next-number", authenticate, async (req, res) => {
  try {
    ...
    logger.audit("Peek next DAO number", req.user?.id, req.ip);
    res.json({ nextNumber: next });
  } catch (error) {
    logger.error("Error generating next DAO number", "DAO_NEXT_NUMBER", {
...
router.get("/:id", authenticate, async (req, res) => {
  try {
    ...
    logger.audit("DAO fetch requested", req.user?.id, req.ip);

    if (!id || id.length > 100) {
      logger.warn("Invalid DAO ID", "DAO_FETCH");
    ...
    if (!dao) {
      logger.warn("DAO not found", "DAO_FETCH");
    ...
    logger.audit("DAO served", req.user?.id, req.ip);
    return void res.json(dao);
  } catch (error) {
    logger.error("Error fetching DAO", "DAO_FETCH", {
      message: String((error as Error)?.message),
    });
    return void res.status(500).json({
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      logger.audit("DAO created successfully", req.user?.id, req.ip);
      ...
      try {
        ...
        const { tplDaoCreated } = await import(
      ...
      logger.error("Failed to create DAO", "DAO_CREATE", {
  ...
  async (req, res) => {
    try {
      ...
      try {
        ...
        // If tasks changed via this endpoint, only FLAG changes (no broadcast here)
      ...
      try {
        ...
        const { tplDaoUpdated } = await import(
          "../services/notificationTemplates"
        );
      ...
      logger.audit("DAO updated successfully", req.user?.id, req.ip);
      res.json(updated);
    } catch (error: any) {
      if (error instanceof z.ZodError) {
        return void res.status(400).json({
      ...
      logger.error("Failed to update DAO", "DAO_UPDATE", {
        message: (error as Error)?.message,
      });
      res.status(500).json({
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      logger.audit("Integrity report generated", req.user?.id, req.ip);
      res.json(report);
    } catch (error) {
      logger.error("Integrity check failed", "DAO_INTEGRITY", {
        message: (error as Error)?.message,
      });
      res.status(500).json({
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      logger.audit("Last DAO deleted successfully", req.user?.id, req.ip);
      return void res.json({
        deletedId: last.id,
        numeroListe: last.numeroListe,
      });
    } catch (error) {
      logger.error("Failed to delete last DAO", "DAO_DELETE_LAST", {
        message: (error as Error)?.message,
      });
      return void res.status(500).json({
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      logger.audit("Tasks reordered successfully", req.user?.id, req.ip);
      res.json(updated);
    } catch (error) {
      logger.error("Failed to reorder tasks", "DAO_TASKS_REORDER", {
        message: (error as Error)?.message,
      });
      res.status(500).json({
  ...
  async (req, res) => {
    try {
      ...
      try {
        ...
        const { tplTaskNotification } = await import(
          "../services/notificationTemplates"
        );
      ...
      logger.audit("Task updated successfully", req.user?.id, req.ip);
      res.json(updated);
    } catch (error) {
      if (error instanceof z.ZodError) {
        return void res.status(400).json({
      ...
      logger.error("Failed to update task", "DAO_TASK_UPDATE", {
        message: (error as Error)?.message,
      });
      res.status(500).json({
