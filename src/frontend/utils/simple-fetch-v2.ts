...
export class SimpleFetchV2 {
  ...
  async fetch(
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      try {
        console.log(
          `🌐 SimpleFetch v2 tentative ${attempt + 1}/${maxRetries + 1} : ${url}`,
        );
        ...
        console.log(`✅ SimpleFetch v2 réussi : ${url} (${response.status})`);
        return response;
      } catch (error) {
        ...
        console.warn(
          `⚠️ SimpleFetch v2 tentative ${attempt + 1} échouée :`,
          errorMessage,
        );
    ...
    console.error(
      `❌ Toutes les tentatives SimpleFetch v2 ont échoué pour : ${url}`,
    );
