  ...
  async fetch(
    ...
    try {
      for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
          console.log(
            `🌐 SimpleFetch tentative ${attempt + 1}/${maxRetries + 1} : ${url}`,
          );

          // Utiliser window.fetch directement pour éviter les problèmes d'iframe
          const response = await window.fetch(url, fetchOptions);

          console.log(`✅ SimpleFetch réussi : ${url} (${response.status})`);
          ...
          console.warn(
            `⚠️ Tentative de simple fetch ${attempt + 1} échouée :`,
            errorMessage,
          );
      ...
      console.error(
        `❌ Toutes les tentatives SimpleFetch ont échoué pour : ${url}`,
      );
