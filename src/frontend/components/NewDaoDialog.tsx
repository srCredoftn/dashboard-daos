...
// Fallback function for local number generation
...
export default function NewDaoDialog({
  ...
  // Fetch users and compute a preview number locally when dialog opens
  useEffect(() => {
    if (open) {
      ...
      fetchData();
      // Always compute a non-mutating preview number locally to avoid consuming server-side sequences
  ...
  // Build unique list of available team members (dedupe by id)
      ...
      case "dateDepot":
        if (!value) {
          ...
          if (selectedDate < today) {
            error = "La date de dépôt ne peut pas être dans le passé";
          }
        }
        break;
      case "teamLeader":
  ...
  const handleFieldChange = (fieldName: string, value: any) => {
    ...
    // Validate field on change with a small delay to avoid constant validation during typing
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Ensure numeroListe is set without hitting a mutating endpoint
      const ensuredNumero =
  ...
  const setTeamLeader = (member: TeamMember | null) => {
    setFormData((prev) => ({
      ...
      // Remove from team members if they were there
      teamMembers: member
      ...
      <DialogContent className="max-w-[95vw] sm:max-w-2xl max-h-[90vh] overflow-y-auto mx-2 sm:mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            {/* Create New Team Member - Hidden per user request */}
            {false && (
